...
The switch produces a Gray Code signal on the clock and data pins which should be easyly be decoded even without a HW unit.  
That's the theory which cause me quite a hadache up to the point a use a scope to have a look at the signals.
Most encoders are of quite poor quality. They seem to be implemented with wiper contancts. 
The wiper seems to loose connection when the knob is turned.
This leads to an almost undecodable noise. The counter seems to jump uncontrollable back and forth. 
...
During interruptions the capacitor is charged through the pull up resistor.
...
### SAM D21
Interrupts are controlled through miltiplexer A, Interrupt lines 0-15 Are assigned to PA/PB 0-15 und 16-32. 
